...
def run_read_only_tests(tape=False, parallel=2):
    ...
    assert (
        storage.SEs[0].get_logical_path(storage.SEs[0].get_storage_path("/test"))
        == "/test"
    )
    ...
    with temp_dir() as tempdir:
        ...
        assert (
            dm.interactive.get(
                testdir, tempdir, recursive="test[12]\.txt", parallel=parallel
            )
            == 0
        )
        assert os.path.isfile(filename)
    ...
    with temp_dir() as tempdir:
        ...
        with no_output(True):
            assert (
                ...
                != 0
            )  # There are some deliberate failures here
        assert os.path.isfile(filename)
    ...
    with no_output(True):
        ...
        assert (
            cmd.ls.run_from_cli("abc") == False
        )  # This should not work, but not throw exception
        assert (
            cmd.ls.run_from_cli('"abc', _return=True) != 0
        )  # This should not work, but not throw exception
        ...
        with fake_argv(["%s-ls" % (dm._branding), "/abcxyz"]):
            assert (
                cmd.ls.run_from_console() != 0
            )  # This should not work, hence the not 0 return value
...
def run_read_write_tests(tape=False, parallel=2):
    ...
    with no_output():
        assert (
            dm.interactive.replicate(
                testdir, testSEs[1], recursive=r"^test[1]\.t.t$", verbose=True
            )
            == 0
        )
        assert (
    ...
    with no_output():
        assert (
            dm.interactive.remove(testdir, testSEs[1], recursive=True) == 0
        )  # Remove everything from SE1
